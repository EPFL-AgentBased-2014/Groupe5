;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;PROJET VIRUS : Justine, Praveen et Kilian;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; 2 different breeds: viruses and houses. Viruses can move and houses are motionless. The houses are here just for graphic reasons.
 
breed [ viruses virus]

breed [ houses house ]

;; lifetime is a virus variable
viruses-own [ lifetime ]
...
to initialize
  ...
  ask patches[
    set pcolor green
    if  ((pxcor - 1) mod ( 10 - city_size )) = 0 or ((pxcor + 1) mod ( 10 - city_size )) = 0 or ((pycor - 1) mod ( 10 - city_size ) ) = 0 or ((pycor + 1) mod ( 10 - city_size )) = 0 [
     set pcolor black 
    ]
    if  (pxcor mod ( 10 - city_size )) = 0 or (pycor mod ( 10 - city_size )) = 0 [
     set pcolor blue 
    ]
  ]
  
  ;; In the beginning, one virus is created in the center. Its shape is a dot and color black.
  create-viruses 1
  ask viruses [
    setxy 0 0 
    set color black
  ...
  ;; Houses are created randomly on green patches and their color is brown.
  let x 0
  while [x < 200] [
    let GreenPatches one-of patches with [ pcolor = green ]
    create-houses 1 [ 
      set x x + 1
      set shape "house"
      set color brown
      set size 2
      setxy [pxcor] of GreenPatches [pycor] of GreenPatches
    ] 
  ]
...
to go
  ...
  mutate
  contagion
  let infected-pop ((count patches with [ pcolor = yellow ]) / (count patches with [ pcolor = yellow or pcolor = black or pcolor = white]))
  if ticks > 300[
    stop-virus
    if infected-pop = 0 [ stop ]
  ]
...
to move
  ...
  let destination one-of neighbors with [pcolor = blue  ]
  move-to destination
...
to contaminate
  ...
  ;; Black turns into yellow depending on alpha. Yellow represents the "infected" population.
  ask neighbors4 [
      if pcolor = black [
...
to contagion
  ...
  let number-of-contamined count patches with [ pcolor = yellow ]
  let t 0
  while [t < number-of-contamined ]  [  
    let i random-float 1
    if i <= beta [
      create-viruses 1 [
        setxy [pxcor] of bluePatch [pycor] of bluePatch 
        set color black 
        set shape "dot"
        set size 1]
    ]
...
to mutate
  ...
  set lifetime-patch lifetime-patch + 1
  if lifetime-patch > immunity * 80 [    set pcolor black
    set lifetime-patch 0
    ]
  ]
...
((count patches with [ pcolor = yellow ]) / (count patches with [ pcolor = yellow or pcolor = black or pcolor = white ])) * 100
0
1
15
...
"" ""
PENS
"infected population" 1.0 0 -16777216 true "" "plot ((count patches with [ pcolor = yellow ]) / (count patches with [ pcolor = yellow or pcolor = black or pcolor = white])) * 100"
"costs" 1.0 0 -2674135 true "" "plot (cost + (treatment-level - 1) * (treatment-level - 1) * (treatment-level - 1) * 1000) / 1000"
...
496
Cost for the City
cost + (treatment-level - 1) * (treatment-level - 1) * (treatment-level - 1) * 1000
0
1
15
...
196
396
Risk for a virus to infect a home
15
130.0
1
...
196
525
Risk for a home to free a virus
15
130.0
1
...
561
Cost per-capita
(cost + (treatment-level - 1) * (treatment-level - 1) * (treatment-level - 1) * 1000)/(count patches with [ pcolor = black or pcolor = yellow or pcolor = white])
0
1
15
...
Line -2674135 false 150 120 150 45
Line -2674135 false 150 180 150 255
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <metric>ticks</metric>
    <metric>((count patches with [ pcolor = yellow ]) / (count patches with [ pcolor = yellow or pcolor = black or pcolor = white]))</metric>
    <metric>count viruses</metric>
    <metric>cout</metric>
    <enumeratedValueSet variable="taille_ville">
      <value value="0"/>
      <value value="1"/>
      <value value="6"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="treatment-level">
      <value value="1"/>
      <value value="1"/>
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="immunity">
      <value value="1"/>
      <value value="1"/>
      <value value="3"/>
    </enumeratedValueSet>
